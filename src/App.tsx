...
import { FileDownIcon, FileUpIcon, Grid2X2Icon, MoonIcon, PinIcon, SunIcon, TrashIcon } from 'lucide-react';
...
function App() {
  ...
  const [title, setTitle] = useState<string>(() => {
    try { return localStorage.getItem('dash-panel-title') ?? 'Dash Panel'; }
    catch { return 'Dash Panel'; }
  });

  useEffect(() => {
    try { localStorage.setItem('dash-panel-title', title); }
    catch { /* ignore */ }
  }, [title]);
  ...
  useEffect(() => {
    ...
    try { localStorage.setItem('dash-panel-theme', isDark ? 'dark' : 'light'); }
    catch { /* ignore */ }
  }, [isDark]);
